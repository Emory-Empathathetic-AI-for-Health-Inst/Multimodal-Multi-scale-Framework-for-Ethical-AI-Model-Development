...
def extract_headers(f_list_elem):
    ...
    kv.append(
        ("file", f_list_elem[1])
    )  # adds my custom field with the original filepath
...
def extract_images(filedata, i, png_destination, flattened_to_level, failed, is16Bit):
    ...
    try:
        ...
        imName = os.path.split(filedata.iloc[i].loc["file"])[1][
            :-4
        ]  # get file name ex: IM-0107-0022
        ...
        if flattened_to_level == "patient":
            ...
            ID1 = filedata.iloc[i].loc[
                "PatientID"
            ]  # Unique identifier for the Patient.
            try:
                ID2 = filedata.iloc[i].loc[
                    "StudyInstanceUID"
        ...
        pngfile = (
            ...
            + folderName
            + "/"
            + hashlib.sha224(imName.encode("utf-8")).hexdigest()
            + ".png"
        )
        ...
        fail_path = (
            filedata.iloc[i].loc["file"],
            failed
            + "1/"
            + os.path.split(filedata.iloc[i].loc["file"])[1][:-4]
            + ".dcm",
        )
...
def execute(
    ...
    ):  # use the specified number of cores to avoid high ram usage
    ...
    if os.path.isfile(pickle_file):
        ...
        )  # search the folders at the depth we request and finds all dicoms
        ...
        if print_images:
            ...
            for i in range(len(filedata)):
                (fmap, fail_path, err) = extract_images(
                    filedata, i, png_destination, flattened_to_level, failed, is16Bit
                )
                if err:
                    count += 1
...
if __name__ == "__main__":
    ...
    ap.add_argument("--DICOMHome", type=str, default="")
    ap.add_argument("--OutputDirectory", type=str, default="")
    ap.add_argument("--Depth", type=int, default=3)
    ap.add_argument("--SplitIntoChunks", type=int, default=10)
    ap.add_argument("--PrintImages", type=str, default="true")
    ap.add_argument("--CommonHeadersOnly", type=str, default="false")
    ap.add_argument("--UseProcesses", type=int, default=0)
    ap.add_argument("--FlattenedToLevel", type=str, default="patient")
    ap.add_argument("--is16Bit", type=str, default="true", choices=("true", "false"))
    ap.add_argument("--SendEmail", type=str, default="false", choices=("true", "false"))
    ap.add_argument("--YourEmail", type=str, default="")
