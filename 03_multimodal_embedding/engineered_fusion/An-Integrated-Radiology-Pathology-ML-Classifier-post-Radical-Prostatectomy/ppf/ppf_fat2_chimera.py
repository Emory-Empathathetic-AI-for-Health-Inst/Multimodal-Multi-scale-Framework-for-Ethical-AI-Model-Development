#!/usr/bin/env python3
# ppf_features_chimere.py — fixed for Chimere naming: {case_id}_0001_t2w.nii.gz
...
def first_order_stats(arr: np.ndarray) -> dict:
    ...
    if arr.size == 0:
        return {"mean": np.nan, "std": np.nan, "median": np.nan,
                "p10": np.nan, "p90": np.nan, "skew": np.nan, "kurt": np.nan}
...
# =============================================================================
# File matching — Chimere pattern: {case_id}_0001_t2w.nii.gz
# =============================================================================
...
def find_matching_file(case_id: str, search_dir: Path) -> Path | None:
    ...
    # 1. Primary Chimere pattern
    p = search_dir / f"{cid}_0001_t2w.nii.gz"
    if p.exists():
        return p
    ...
    # 3. Walk: stem starts with case_id followed by a non-alphanumeric character
    #    (space, underscore, hyphen, dot, etc.) or end of stem.
    #    Matches PAT00170.nii.gz, PAT00170_0001_t2w.nii.gz,
    #            "PAT00170 mri dec 2019.nii.gz", PAT00170-mri.nii.gz, etc.
    #    Does NOT match PAT001701.nii.gz (would be ambiguous with PAT00170).
    pattern = re.compile(rf"^{re.escape(cid)}(?![A-Za-z0-9])")
...
def ppf_t2w_features(t2w_norm, ppf_mask):
    ...
    edge  = np.sqrt(
        sobel(t2w_norm, axis=0) ** 2 +
        sobel(t2w_norm, axis=1) ** 2 +
        sobel(t2w_norm, axis=2) ** 2
    )
...
def extract_features(case_id: str, t2w_dir: Path, ppf_dir: Path, tzpz_dir: Path | None):
    t2w_path  = find_matching_file(case_id, t2w_dir)
    ppf_path  = find_matching_file(case_id, ppf_dir)
    tzpz_path = find_matching_file(case_id, tzpz_dir) if tzpz_dir else None

    print(f"  T2W  : {t2w_path  or 'MISS'}")
    print(f"  PPF  : {ppf_path  or 'MISS'}")
    ...
    ref_mask, ref_source = build_ref_mask(t2w_img, tzpz_path)
    if ref_mask is None:
        return None, "missing_or_empty_ref_mask"

    t2w_norm = robust_normalize_t2w(t2w_arr)

    rec = {"case_id": case_id, "prostate_ref_source": ref_source}
...
def main():
    ap = argparse.ArgumentParser(
        description="PPF feature extraction — Chimere dataset."
    )
    ...
    id_col = args.id_col
    if id_col is None:
        for candidate in ["case_id", "patient_id", "PatientID", "pid", "id"]:
            if candidate in df.columns:
                id_col = candidate
                break
    if id_col is None:
        sys.exit(f"Could not detect ID column. Columns: {list(df.columns)}")
    ...
    for _, row in df.iterrows():
        ...
        print(f"\n[{case_id}]")
        rec, reason = extract_features(case_id, t2w_dir, ppf_dir, tzpz_dir)
    ...
    skipped_path = (
        Path(args.skipped_csv) if args.skipped_csv
