...
Geometry features (per left/right/total):
  - volume (mL), surface area, sphericity
  - elongation, flatness, major/minor axis, axis ratio (PCA)
  - asymmetry indices (volume, surface area, major axis)
  - bounding box dimensions (W x H x D in mm)
  - centroid distance L-R, L-R split confidence

T2W texture features (per left/right/total SV mask, optional):
  - mean, std, median, p10, p90, range, skewness, kurtosis
  - GLCM: contrast, homogeneity, energy, correlation, ASM
  - gradient magnitude: mean, std (edge sharpness)
  - SV-to-background intensity ratio (normalizes scanner differences)
  - left/right intensity asymmetry

Prostate features (optional, SV-relative only):
  - prostate volume (mL)
  - SV/prostate volume ratio
  - SV centroid to prostate centroid distance (mm)
...
def normalize_case_stem(stem: str) -> str:
    """
    Normalize file stems across naming styles.

    Examples:
    10106411_t2w         -> 10106411
    10106411_0000        -> 10106411
    10106411_t2w_0000    -> 10106411
    """
    stem = re.sub(r'_\d{4}$', '', stem)                  # remove _0000
    stem = re.sub(r'_(t2w|adc|dwi|hbv)$', '', stem, flags=re.IGNORECASE)
    return stem 
...
def strip_channel_suffix(stem: str) -> str:
    """
    Remove nnUNet-style channel suffixes and modality suffixes.
...
def find_matching_file(case_id: str, search_dir: str) -> Optional[str]:
    ...
    Handles nnUNet _0000 channel suffixes on either side of the match.

    Matches:
      case_id '10005_1000005' <- file '10005_1000005_0000.nii.gz'  OK
      case_id '10005_1000005' <- file '10005_1000005.nii.gz'       OK
      case_id '10005_1000005_0000' <- file '10005_1000005.nii.gz'  OK
    """
    case_id_clean = strip_channel_suffix(case_id)

    # 1. exact match (no channel suffix)
    for ext in [".nii.gz", ".nii"]:
        p = os.path.join(search_dir, f"{case_id_clean}{ext}")
        if os.path.exists(p):
            return p

    # 2. explicit _0000 suffix
    for ext in [".nii.gz", ".nii"]:
        p = os.path.join(search_dir, f"{case_id_clean}_0000{ext}")
        if os.path.exists(p):
            return p

    # 3. walk and match by cleaned stem
    for p in iter_nii_files(search_dir):
        file_stem = strip_nii_ext(os.path.basename(p))
        if strip_channel_suffix(file_stem) == case_id_clean:
            return p
...
def t2w_intensity_features(
    ...
    Parameters
    ----------
    binary_zyx     : foreground mask (z,y,x bool/int)
    t2w_arr        : T2W array resampled to mask space (z,y,x float32)
    background_arr : same T2W array used for background ratio (pass full array)
    """
    nan_keys = [
...
def main():
    ...
    base_cols  = ["case_id", "sv_mask_path", "t2w_path", "sv_label", "derive_lr"]
