...
def make_fixed_train_test_split(
    data: pd.DataFrame,
    label_col: str = "grade",
    ...
    if abs(train_size + test_size - 1.0) > 1e-6:
        raise ValueError("train_size + test_size must sum to 1.0")
    ...
    for seed in range(max_trials):
        ...
        if test_pos >= min_pos_test:
            ...
            print("Train cohort:", len(train_df), "| BCR+ =", int((train_df[label_col] == 1).sum()))
            print("Independent test cohort:", len(test_df), "| BCR+ =", test_pos)

            print("\nTrain cohort label counts:")
...
def build_loaders(args, train_df, test_df):
...
def train_model(args, data, model, device, model_name):
    ...
    train_df, test_df = make_fixed_train_test_split(
        data=data,
        label_col="grade",
        train_size=0.80,
        test_size=0.20,
    ...
    train_loader, test_loader = build_loaders(args, train_df, test_df)
    ...
    with open(os.path.join(out_dir, "run_config.txt"), "w") as f:
        ...
        f.write("independent_test_fraction: 0.20\n")
        f.write("train_fraction: 0.80\n")
        f.write("validation_used: False\n")
    ...
    for epoch in tqdm(range(args.n_epochs)):
        ...
        if train_loss < best_train_loss:
            ...
            ckpt_path = os.path.join(out_dir, f"{model_name}_best_train_loss.pt")
            torch.save(
                {
                    "epoch": epoch,
            ...
            print("Saved best-train-loss model:", ckpt_path)
    ...
    # Evaluate independent test cohort using best training-loss checkpoint
    best_ckpt_path = os.path.join(out_dir, f"{model_name}_best_train_loss.pt")
    ckpt = torch.load(best_ckpt_path, map_location="cpu")
    model.load_state_dict(ckpt["model_state_dict"])
    model.to(device)

    print("\nRunning INDEPENDENT TEST evaluation using BEST training loss checkpoint")
...
if __name__ == "__main__":
    ...
    print("independent_test_fraction: 0.20")
    print("train_fraction: 0.80")
    print("validation_used: False")
    print("==============================================\n")
