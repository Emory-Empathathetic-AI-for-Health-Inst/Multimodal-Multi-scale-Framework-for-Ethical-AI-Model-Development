...
def find_matching_file(case_id: str, search_dir: Path) -> Path | None:
    ...
    Match NIfTI file by case_id with fallbacks:
      1. Exact stem match
      2. With _0000 suffix
      3. Walk + cleaned exact match
      4. Walk + underscore token match
      5. Walk + prefix match for messy names like PAT00053mri June 2015
    """
    if not search_dir or not search_dir.exists():
        return None

    cid = strip_channel_suffix(case_id).strip()
    ...
    for p in iter_nii_files(search_dir):
        stem = strip_channel_suffix(strip_nii_ext(p.name)).strip()
        if stem == cid:
            return p

    # 4. walk + case_id as one underscore-split part
    for p in iter_nii_files(search_dir):
        stem = strip_channel_suffix(strip_nii_ext(p.name)).strip()
        parts = stem.split("_")
        if cid in parts:
    ...
    # 5. walk + prefix match
    # handles: PAT00053 -> PAT00053mri June 2015.nii.gz
    for p in iter_nii_files(search_dir):
        stem = strip_channel_suffix(strip_nii_ext(p.name)).strip()
