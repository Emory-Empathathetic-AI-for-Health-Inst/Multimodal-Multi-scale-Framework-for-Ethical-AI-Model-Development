...
# Fix path format for WSL
base_path = "/PIQUALvsMRQy_study/PIQUALvsMRQy_server/PIQUALvsMRQy_study_server_segmentation_experiment/PIQUALvsMRQy_segmentations/cluster1_t2"

# Get all image and segmentation files
image_files = sorted(glob.glob(os.path.join(base_path, "*_t2w.nii.gz")))
segmentation_files = sorted(glob.glob(os.path.join(base_path, "*_gland.nii.gz")))
...
X, y = load_and_preprocess_data(image_files, segmentation_files)
...
callbacks = [
    ModelCheckpoint(
        'best_vnet_model_cluster1.h5',  # Updated filename for V-Net
        save_best_only=True,
        monitor='dice_coefficient',  # Using dice coefficient as the metric to monitor
        mode='max'
    ),
    ReduceLROnPlateau(
        monitor='dice_coefficient',  # Using dice coefficient for learning rate reduction
        factor=0.2,
        patience=5,
        min_lr=1e-6,
        mode='max'
...
history = model.fit(
    ...
    epochs=100,  # Consider increasing epochs since we're using all data
    callbacks=callbacks
)
...
output_file = os.path.join(os.path.dirname(test_img), "vnet_prediction_" + os.path.basename(test_img))
...
plt.tight_layout()
plt.savefig('vnet_training_history.png')
plt.close()
...
def plot_results(image, true_mask, pred_mask, slice_idx):
    ...
    plt.tight_layout()
    plt.savefig(f'vnet_result_slice_{slice_idx}.png')
    plt.close()
