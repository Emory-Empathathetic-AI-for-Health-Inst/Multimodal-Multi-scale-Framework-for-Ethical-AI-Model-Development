...
from post_processing import treatment_postprocessing
...
if __name__ == "__main__":
    ...
    for i,j in df_test.iterrows():
        for tidx in range(len(treatments)):
            ...
            gt = df_test.at[i, treatments_cols[treatments[tidx]]]
            if gt is not None and gt:
                text = treatments_tokens[treatments[tidx]]+" "+df_test.at[i, "TEXT"]
                label = "<TRUE>"
                gt = 1
            ...
            out = qamodel(input_ids.to(device), attention_mask.to(device))
